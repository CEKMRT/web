...
const TombolStasiun: React.FC<TombolStasiunProps> = ({ onSelectStation }) => {
  return (
    <div>
      <div className="flex flex-wrap justify-between justify-items-start content-center place-content-evenly gap-4 py-4 max-w-sm mx-auto bg-white shadow-md rounded-lg md:max-w-2xl px-4
      ...
      z-10"
        >
          ...
          <Button onClick={() => onSelectStation("Haji Nawi")}>
            Haji Nawi
          </Button>
          <Button onClick={() => onSelectStation("Cipete")}>Cipete Raya</Button>
          <Button onClick={() => onSelectStation("Fatmawati")}>
            Fatmawati
          </Button>
          <Button onClick={() => onSelectStation("Lebak Bulus")}>
            Lebak Bulus Grab
          </Button>
        </div>
      </div>
